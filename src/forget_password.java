...
//    update new question in database
    ...
    void update_questions()
    {
        ...
        try
        {
            ...
            PreparedStatement ps1=con.prepareStatement("update forget_password set security_questions=? where question=? and (id=? or username=?)");
            for(int i=0;i<3;i++)
            {
    ...
    void update_answer() 
    {
        ...
        {            
            Connection con=c.connect();
            PreparedStatement ps1=con.prepareStatement("update forget_password set answers=? where question=? and (id=? or username=?)");
            for(int i=0;i<3;i++)
            {
    ...
    void update_password()
    {
        ...
        {            
            ...
            PreparedStatement p1=con.prepareStatement(" update forget_password set password=? where id=? or username=?");
            p1.setString(1,password1);
            p1.setString(2,username1);
            p1.setString(3,username1);
            p1.executeUpdate(); 
            
            PreparedStatement p2=con.prepareStatement(" update student_details set password=? where id=? or username=?");    
            p2.setString(1,password1);
            p2.setString(2,username1);
            p2.setString(3,username1);
            p2.executeUpdate();   
     
            PreparedStatement p3=con.prepareStatement(" update student_login_info set password=? where id=? or username=?");        
...
//    it will take new recovery answers
    ...
    void check_details()
    {
        ...
        {            
            Connection con=c.connect();
            Statement s=con.createStatement();
            ResultSet rs=s.executeQuery("Select username,id from forget_password ");  
            int i=0;
            while(rs.next())
            {
    ...
    public void show_questions()
    {
        ...
        {            
            Connection con=c.connect();
            PreparedStatement p=con.prepareStatement("select security_questions from forget_password where id = ? or username=?");
    ...
    private void initComponents() {
        ...
        jPanel1.setPreferredSize(new java.awt.Dimension(712, 700));
        ...
        jLabel10.setText("STUDENT ID / USERNAME");
...
//        show_question();
    ...
    if(ty==3) 
    {
        ...
        {            
            Connection con=c.connect();
            PreparedStatement ps=con.prepareStatement("select answers from forget_password where id=? or username=?");
